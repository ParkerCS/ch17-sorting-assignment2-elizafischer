...
print(nba_leader_list[10][1])

def insertion_sort(my_list):
    for pos in range(1, len(my_list)):
        key_pos = pos
        scan_pos = key_pos - 1
        key_val = my_list[key_pos][1]
        key_val_both = my_list[key_pos]
        while my_list[scan_pos][1] < key_val and scan_pos >= 0: #and my_list[scan_pos][0] > key_val:
            my_list[scan_pos + 1] = my_list[scan_pos]
            scan_pos -= 1
        my_list[scan_pos + 1] = key_val_both
    return my_list

print("\nInsertion Sort")
insertion_sort(nba_leader_list)
